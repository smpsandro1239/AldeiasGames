...
export function ClienteDashboardView({ user, eventos, participacoes, onParticipar, onRevelar }: ClienteDashboardViewProps) {
  ...
  return (
    <div className="space-y-8">
      <div className="bg-gradient-to-r from-indigo-600 to-violet-700 rounded-3xl p-8 text-white relative overflow-hidden">
        ...
        <p className="text-indigo-100 mb-6 relative z-10">Tens {participacoes.filter((p) => !p.revelada).length} raspadinhas por revelar.</p>
      ...
      {activeTab === 'explorar' ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {eventos.flatMap(ev => ev.jogos || []).map((jogo) => (
            <GameCard key={jogo.id} jogo={jogo} onParticipar={onParticipar} />
          ))}
        </div>
      ) : (
            ...
            <UICard key={p.id} className="p-4 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${p.revelada ? 'bg-gray-100' : 'bg-indigo-100 text-indigo-600'}`}>
                  {p.jogo?.tipo === 'raspadinha' ? <Trophy className="w-6 h-6" /> : <Star className="w-6 h-6" />}
                </div>
                <div>
                  <h4 className="font-bold">{p.jogo?.titulo || 'Participação'}</h4>
